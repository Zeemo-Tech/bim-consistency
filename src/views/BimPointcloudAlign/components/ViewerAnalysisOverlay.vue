...
const pointRecords = computed(
  () => props.points ?? (props.point ? [props.point] : []),
)
const distanceRecords = computed(
  () => props.distances ?? (props.distance ? [props.distance] : []),
)
...
<template>
  <div v-if="props.mode !== 'none'" class="analysis-overlay">
    <div class="analysis-toolbar">
      ...
      <span v-if="props.mode === 'distance' && distanceRecords.length === 0">
        依次点击两点完成一段测距
      </span>
      <span v-else-if="props.mode === 'locate' && pointRecords.length === 0">
        点击 BIM 或点云中的任意位置拾取坐标
      </span>
      <span v-else-if="props.mode === 'area' && areaRecords.length === 0">
        连续点击至少三个点，双击首点闭合区域
      </span>
      <span v-else-if="props.mode === 'distance'" class="analysis-value">
        ...
        <small v-if="latestDistanceMetrics">
          ...
          垂直 {{ latestDistanceMetrics.verticalDistance.toFixed(3) }} m · 坡度
          {{ latestDistanceMetrics.slopeDegrees.toFixed(2) }}°
        </small>
      </span>
      <span v-else-if="props.mode === 'area'" class="analysis-value">
        {{ areaRecords.length }} 个区域
      </span>
      <span v-else class="analysis-value">
        {{ pointRecords.length }} 个点，最近 {{ pointText }}
      </span>
