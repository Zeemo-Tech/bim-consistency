  ...
  >
    ...
    >
      <svg viewBox="0 0 16 16" aria-hidden="true">
        <path
          d="M2.5 7.2 8 2.8l5.5 4.4V13a.8.8 0 0 1-.8.8H9.2V9.6H6.8V13.8H3.3A.8.8 0 0 1 2.5 13V7.2z"
        />
      </svg>
    </button>
    <div class="view-cube-roll">
      <button
        ...
        aria-label="逆时针旋转 90°"
        @pointerdown.stop
        @click.stop="emit('roll', -1)"
      >
        <svg viewBox="0 0 16 16" aria-hidden="true">
          <path
            d="M12.4 10.6a5.2 5.2 0 0 0-8.6-3.8"
            fill="none"
            stroke="currentColor"
            stroke-width="1.5"
            stroke-linecap="round"
          />
          <path d="M2.4 7.6 6.1 5.8 4.6 9.6z" fill="currentColor" />
        </svg>
      </button>
      <button
        type="button"
      ...
      >
        <svg viewBox="0 0 16 16" aria-hidden="true">
          <path
            d="M3.6 10.6a5.2 5.2 0 0 1 8.6-3.8"
            fill="none"
            stroke="currentColor"
            stroke-width="1.5"
            stroke-linecap="round"
          />
          <path d="M13.6 7.6 9.9 5.8 11.4 9.6z" fill="currentColor" />
        </svg>
      </button>
    </div>
  </div>
...
function hitRegion(
  event: PointerEvent,
): { direction: THREE.Vector3; id: string; faces: number[] } | null {
  ...
  if (hit.object.userData.direction) {
    ...
    return {
      direction,
      id: hit.object.userData.id as string,
      faces: directionFaceIndices(direction),
    }
  }
...
function handlePointerMove(event: PointerEvent) {
  ...
  if (dragging) {
    emit('orbit', {
      lon: -event.movementX * (180 / SIZE),
      lat: event.movementY * (180 / SIZE),
    })
    event.preventDefault()
    event.stopPropagation()
    return
    ...
    for (const raw of directions) {
      ...
      for (const layout of plateLayouts(raw, kind)) {
        const mesh = new THREE.Mesh(
          new THREE.BoxGeometry(...layout.size),
          regionMaterial(),
        )
