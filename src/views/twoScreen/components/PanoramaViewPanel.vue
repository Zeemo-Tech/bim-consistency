<template>
  <div class="panorama-view-panel">
    ...
    <div
      ...
      :class="{ 'is-collapsed': timelineCollapsed }"
    >
      <button
        class="timeline-collapse-btn"
        type="button"
        :aria-label="timelineCollapsed ? '展开时间条' : '收起时间条'"
        :title="timelineCollapsed ? '展开时间条' : '收起时间条'"
        @click="toggleTimelineCollapse"
      >
        <el-icon v-if="!timelineCollapsed"><ArrowLeft /></el-icon>
        <el-icon v-else><ArrowRight /></el-icon>
      </button>

      <div v-show="!timelineCollapsed" class="timeline-overlay__body">
        <TimeRangeStatusTimeline
          :overall-range="timeRange"
...
import TimeRangeStatusTimeline from './TimeRangeStatusTimeline.vue'
...
const emit = defineEmits<{
  ...
  (e: 'visible-change', value: TimeRange): void
  (e: 'segment-click', value: Segment): void
}>()
...
const nowValue = computed(() => props.now ?? Date.now())

const resolvedSegments = computed(() => props.segments ?? [])

const currentYawDegrees = computed(() =>
  resolveTrajectoryHeadingDegrees(currentImageInfo.value ?? {}),
)

const handleVisibleChange = (range: TimeRange) => {
...
.timeline-overlay__body :deep(.time-range-timeline) {
  position: static;
  left: auto;
  bottom: auto;
  width: 100%;
