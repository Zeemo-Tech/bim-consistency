...
export const organizationIdKey = "organization-id";
/**
...
export function setToken(data: DataInfo<Date>) {
  ...
  function setUserKey({ avatar, username, nickname, email, roles, permissions }) {
...
export function removeToken() {
  ...
  storageLocal().removeItem(organizationIdKey);
}

/** 获取当前组织ID */
export function getOrganizationId(): number | null {
  const id = storageLocal().getItem<number>(organizationIdKey);
  return id || null;
}
...
export function setOrganizationId(id: number | null) {
  if (id) {
    storageLocal().setItem(organizationIdKey, id);
    useUserStoreHook().SET_ORGANIZATIONID(id);
  } else {
    storageLocal().removeItem(organizationIdKey);
    useUserStoreHook().SET_ORGANIZATIONID(null);
  }
}
