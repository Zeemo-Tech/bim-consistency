...
> {
  ...
  setBounds(box: THREE.Box3) {
    ...
    this.material.uniforms.gridOrigin!.value.set(
  ...
  updateForCamera(camera: THREE.Camera) {
    ...
    this.material.uniforms.fadeDistance!.value = (
    ...
    this.material.uniforms.gridLevel!.value = Math.max(
      0,
      Math.log10(
        Math.max(1, viewHeight / (this.material.uniforms.cellSize!.value * 80)),
      ),
    )
